(ns isara.main
  "Kotodama WASM entrypoint for the Isara clean-room actor (L4) — Clojure port.
  ...
  facts (`isara.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"isara.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "isara-compat" :tier tier :entities entities} 200])
